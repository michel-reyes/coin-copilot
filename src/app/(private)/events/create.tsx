...
export default function CreateEventScreen() {
    ...
    const handleSubmit = async () => {
        ...
        if (
            ...
            (!recurrenceInterval || parseInt(recurrenceInterval) <= 0)
        ) {
        ...
        try {
            const schedules: CreateNotificationScheduleData[] =
    ...
    const removeNotificationSchedule = (index: number) => {
        if (notificationSchedules.length > 1) {
            setNotificationSchedules(
                notificationSchedules.filter((_, i) => i !== index)
            );
        } else {
            Alert.alert(
                'Error',
                'You must have at least one notification time'
            );
        }
    };
                    ...
                    {notificationSchedules.map((schedule, index) => (
                        <View key={index} style={styles.notificationSchedule}>
                            ...
                            {notificationSchedules.length > 1 && (
                                <TouchableOpacity
                                    onPress={() =>
                                        removeNotificationSchedule(index)
                                    }
